...
// --- Initialize Services ---
const pumps = new Map<number, PumpStateMachine>();
...
async function initTanksFromPOS() {
    try {
        const serviceToken = jwt.sign({ service: 'pump-controller' }, pumpSecret, { expiresIn: '1h' });
        
        const res = await axios.get(`${posApiUrl}/fuel-config/active`, {
            headers: { Authorization: `Bearer ${serviceToken}` },
            timeout: 5000,
        });

        const configs: TankConfig[] = res.data.map((fc: any) => ({
...
function getPump(req: express.Request, res: express.Response): PumpStateMachine | null {
...
app.post('/api/pump/:pumpId/authorize', (req, res) => {
    ...
    if (token) {
        try { console.log(`[Pump ${pump.pumpId}] Auth by:`, jwt.decode(token)); } catch (e) {}
    }
...
app.post('/api/tank/refuel', (req, res) => {
    ...
    if (!fuel_type) return res.status(400).json({ message: 'fuel_type is required' });
    ...
    for (const [, pump] of pumps) {
        if (pump.getState() === 'IDLE') pump.lock();
    }
...
app.post('/api/tank/refuel/cancel', (req, res) => {
    ...
    if (!tankManager.isAnyRefueling()) {
        for (const [, pump] of pumps) {
            if (pump.getState() === 'LOCKED') pump.unlock();
        }
    }
